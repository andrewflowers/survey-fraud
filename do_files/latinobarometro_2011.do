use "C:\Users\noblek\Google Drive\Duplication Data\Data for Analysis\latinobarometro_2011.dta", clear
cd "C:\Users\noblek\Google Drive\Duplication Data\Do files for each dataset\working folder\"

*record initial variable count, dataset name
local orig_dat_vars `c(k)'
local dataset "latinobarometro_2011"
local file_for_analysis = "`dataset'" + "_temp" + ".dta"

*create clean country variable
decode idenpa, gen(country_var)
...
drop numinves - LOCAL10 S1NICC7 - country_var
...
forvalues i=1/18 {
	...
	*remove variables with >=10% missing data 
	foreach newvar of varlist P1ST - P86N {
		quietly fre `newvar'
