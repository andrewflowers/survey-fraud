use "worldvaluessurvey_wave3.dta", clear

*record initial variable count, dataset name
local orig_dat_vars `c(k)'
local dataset "worldvaluessurvey_wave3"
local file_for_analysis = "`dataset'" + "_temp" + ".dta"

*create clean country variable
decode V2, gen(country_var)
replace country_var = country_var + "_2" if V2B==2
...
forvalues i=1/56 {
	...
	*remove variables with >=10% missing data 
	foreach newvar of varlist V4 - V213 {
		quietly fre `newvar'
