use "afrobarometer_wave2.dta", clear


*record initial variable count, dataset name
local orig_dat_vars `c(k)'
local dataset "afrobarometer_wave2"
local file_for_analysis = "`dataset'" + ".dta"

*remove id, demographic, weight, and metadata Variables
drop respno - strtime q80 - filter__
...
forvalues i=1/16 {
	...
	*remove variables with >=10% missing data 
	foreach newvar of varlist q1a - q77 {
		quietly fre `newvar'
		if (`r(N_valid)'/`r(N)')<.9{
			 drop `newvar'
			}
