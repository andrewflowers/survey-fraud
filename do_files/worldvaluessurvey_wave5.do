use "worldvaluessurvey_wave5.dta", clear

-set more off-

*record initial variable count, dataset name
local orig_dat_vars `c(k)'
local dataset "worldvaluessurvey_wave5"
local file_for_analysis = "`dataset'" + "_temp" + ".dta"

*create clean country variable
decode V2A, gen(country_var)
replace country_var = country_var + "_ballot_a" if V1B==1
replace country_var = country_var + "_ballot_b" if V1B==2
...
drop V1 - V3 V235 - country_var

*record non-demographic variable count
local substantive_dat_vars `c(k)'

*create missing Code
foreach var of varlist V4 - V234{
	quietly replace `var' = . if `var'<=-4
}
...
forvalues i=1/73 {
	...
	*remove variables with >=10% missing data 
	foreach newvar of varlist V4 - V234 {
		quietly fre `newvar'
