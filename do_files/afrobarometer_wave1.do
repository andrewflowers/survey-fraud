use "afrobarometer_wave1.dta", clear


*record initial variable count, dataset name
local orig_dat_vars `c(k)'
local dataset "afrobarometer_wave1"

*remove id, demographic, weight, and metadata Variables
drop refnumb casenumb occup - povinc withinwt - area

*record non-demographic variable count
local substantive_dat_vars `c(k)'

*create Missing Code
foreach var of varlist identity - xstronld{
	quietly replace `var' = . if `var'==99
}

*create temporary data for analysis
save tempafrobarometer_wave1.dta, replace

forvalues i=1/12 {
	use tempafrobarometer_wave1.dta, clear
	...
	*remove variables with >=10% missing data 
	foreach newvar of varlist identity - xstronld {
		quietly fre `newvar'
		if (`r(N_valid)'/`r(N)')<.9{
			 drop `newvar'
			}
	...
	*saving histogram
	local graph_name = "`dataset'" + "_" + "`countryname'"
	histogram match_percent, xlabel(.4(.2)1) saving("`graph_name'")
	}
